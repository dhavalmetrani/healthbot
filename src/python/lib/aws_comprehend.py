...
class LanguageEngine:
  ...
  def get_dominant_language(self, text):
    try:
      json_obj = self._comprehend.detect_dominant_language(
        Text=text)
      result = json.dumps(json_obj['Languages'], sort_keys=True, indent=2)
  ...
  def get_key_phrases(self, text):
    try:
      ...
      result = json.dumps(json_obj['KeyPhrases'], sort_keys=True, indent=2)
  ...
  def get_entities(self, text):
    try:
      ...
      for entity in entities:
        txt += f'{json.dumps(entity, sort_keys=True, indent=2)}\n'
    except Exception as e:
      txt = e
    finally:
      return txt.strip()
